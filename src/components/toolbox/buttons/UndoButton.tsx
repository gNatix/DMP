...
export const undoButtonConfig: ToolButtonConfig = {
  ...
  enabled: true,
  category: 'history',
  weight: 1, // First in history category
...
const UndoButton = ({ onUndo, canUndo }: UndoButtonPropsExtended) => {
  ...
  return (
    ...
    >
      ...
      >
        {undoButtonConfig.icon}
      </button>
      <span className="text-[9px] text-gray-500 font-medium mt-0.5">{undoButtonConfig.shortcutKey}</span>
      <div 
      ...
      >
        {undoButtonConfig.label} ({undoButtonConfig.shortcutKey})
      </div>
    </div>
  );
};
